...
public class CSVDataTransformer {
	...
	public DBObject transform(String csv) {
		...
		if (rawdata.length != 31 && rawdata.length != 32) {
			throw new IllegalArgumentException("CSV input is not of the correct length. It should contain either 31 or 32 values.");
		}
		...
		try {
			...
			throw new IllegalArgumentException("The first value is not a correct ISO date format. It cannot be converted to a Date object.");
		}
		...
		pvData.put("temp", Float.parseFloat(rawdata[9]));
